...
ALabel::ALabel(int x_pos, int y_pos, int width, int height, const AFont &font, const AColor &color)
	: X_Pos(x_pos), Y_Pos(y_pos), Width(width), Height(height), Font(font), Color(color), Last_Char(0)
{
...
{
	//Поднимаем регистр английских букв
	if (symbol >= L'a' && symbol <= L'z')
		symbol -= L'a' - L'A';
	
	//Поднимаем регистр кириллических букв другой вариант
	if (symbol >= 0x430 && symbol <= 0x44f)
		symbol -= 0x20;

	if (symbol >= 0x450 && symbol <= 0x45f)
		symbol -= 0x50;
	//Поднимаем регистр кириллических букв
