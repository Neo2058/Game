...
//------------------------------------------------------------------------------------------------------------
AsLevel::AsLevel()
: Level_Rect{}, Need_To_Cancel_All(false), Next_Level_Number(0), Parachute_Color(AsConfig::Red_Color, AsConfig::Blue_Color, AsConfig::Global_Scale),
  Advertisement(0), Available_Bricks_Count(0), Current_Level_Number(0)
{
	Level = this;
}
//------------------------------------------------------------------------------------------------------------
...
void AsLevel::Init()
{
	...
	for (int i = 0; i < ALevel_Data::Max_Level_Number; i++)
	{
		...
		if (i == 7)
			level_data->Advertisment = new AAdvertisement(4, 7, 2, 3);
		
		if (i == 9)
			level_data->Advertisment = new AAdvertisement(1, 9, 2, 3);
	}
...
void AsLevel::Set_Current_Level(int level_number)
{
	...
	Advertisement = level_data->Advertisment;
...
{// Возврат: истина\ложь закончилась очистка
...
bool AsLevel::Has_Brick_At(RECT &monster_rect)
{
	...
	//Т.к. ячейка уровня больше кирпича (хотя и начинается в одинаковых координатах), то она имеет правую и нижнюю пустую полосу,
	//в которой может находится монстр. Игнорируем ряд(или столбец) кирпичейб если монстр попал в ячейку но не попал в кирпич.
...
//------------------------------------------------------------------------------------------------------------
void AsLevel::Delete_Objects(vector<AGraphics_Object*>& falling_letters)
{
...
//------------------------------------------------------------------------------------------------------------
void AsLevel::Act_Objects(vector<AGraphics_Object *> &falling_letters)
{
