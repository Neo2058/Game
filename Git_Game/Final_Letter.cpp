...
AFinal_Letter::AFinal_Letter(double x_pos, double y_pos, const wchar_t letter, int width, int height)
	: X_Pos(x_pos), Y_Pos(y_pos), Letter(letter), Width(width), Height(height), Final_Letter_State(EFinal_Letter_State::Show_Letter), Got_Letter_Size(false),
	  Letter_Rect{}
{
...
void AFinal_Letter::Draw(HDC hdc, RECT& paint_area)
{
	...
	case EFinal_Letter_State::Show_Letter:
		...
		if (!Got_Letter_Size)
		{
			...
			Width = letter_size.cx + letter_size.cx / 2; //133% от ширины. Стирает хвостики у курсивных букв выступающих за область ихи отрисовки
