...
bool ABall::Detect_Hits_Cycling()
{
	...
	if (Recent_Hits_Pos >= buffer_len)
		Recent_Hits_Pos = 0; //Зацикленный буфер

	++Recent_Hits_Count;

	if (Recent_Hits_Count < buffer_len)
		return false;//Не подсчитывает повторения, пока не наберем buffer_len столкновений

	//Техника сканирования "окнами" по 3 столкновения
